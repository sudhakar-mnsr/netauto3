...
// the program uses the Dailer to explicitly configure the client
// dialing process.

// The program uses -host to specify the remote address
// (or socket path) and -n for the network protocol ("udp" or "datagra,").
...
func main() {
   ...
   // Create a Dailer which allows us to specify dialing options.
   ...
   // ensure we rea 48 bytes back (NTP protocol spec)
   if read != 48 {
